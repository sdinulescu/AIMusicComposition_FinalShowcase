...
class Tree<E> extends PSTNode<E> {
  PSTNode<E> root = new PSTNode<E>("", 0);
  ArrayList<String> passes = new ArrayList<String>(); //contains all nodes that pass r threshold
  ArrayList<Character> singleMotives = new ArrayList<Character>(); //holds single motive chars in string
  ...
  void search(String input, int LValue) { //searches input string for motives
    if (LValue <= 0 || input.length() < LValue) { 
      return;
    } //boundary checks
  ...
  void solveInfiniteLooping(int countBack, int oldCountBack, int times) { 
    ...
    if (isLooping == true) { //if infinite looping occurs for a number past the number "times"
      ...
      System.out.println("No infinite looping occurs");
    }
  }
