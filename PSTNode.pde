...
class PSTNode<E> {
  int index = 0;
  String stringMotives; //string stored for each object
  int count; //counts the number of instances that character is in the input string
  ArrayList<PSTNode<E>> children = new ArrayList<PSTNode<E>>(); //next node
  double empiricalProbability; //stores empirical probability
  double conditionalProbability; //stores conditional probability
  //private ArrayList<PSTNode<E>> temp = new ArrayList<PSTNode<E>>(); //previous node
  PSTNode<E> node = this;

  ArrayList<Double> nextProbs = new ArrayList<Double>(); //holds probabilities of what follows the node


  PSTNode() {
  } //default constructor
  PSTNode(String stringMotives, int count) { //overloaded constructor to create a node with a String and a count
  ...
  void setString(String stringMotives) {
    this.stringMotives = stringMotives;
  }
  String getString() {
  ...
  void setEmpProb(double empiricalProbability) {
    this.empiricalProbability = empiricalProbability;
  }
  double getEmpProb() {
    return empiricalProbability;
  }
  void setCondProb(double conditionalProbability) {
    this.conditionalProbability = conditionalProbability;
  }
  double getCondProb() {
    return conditionalProbability;
  }
  void setNextProbs(ArrayList<Double> next) {
    for (int i = 0; i < next.size(); i++) {
      nextProbs.add(next.get(i));
    }
  }
  ArrayList<Double> getNextProbs() {
    return nextProbs;
  }
  void addChildren(PSTNode<E> node) {//adds the node in the parameter to the arrayList pertaining to that motive
    //System.out.println(node.stringMotives + " " + node.count);
    children.add(node);
  }
  ArrayList<PSTNode<E>> getChildren() { //gets the arrayList pertaining to that motive
    return children;
  }
  boolean hasChildren() {
    if (children.size() != 0) {
  ...
  double calculateEmpProbs(String input, double pMinValue) { //calculates and sets empirical probability for each node
  ...
  ArrayList<String> eliminateCondProbs(String input, int rValue, ArrayList<String> passes) {
  ...
  void calcs(String input, ArrayList<Character> motives, ArrayList<Integer> tally, ArrayList<Double> probabs, ArrayList<Integer> counts) { //calculates probabilities in the tree based on next
    char next;
    int length = 0;
    int probCount = 0; 
    //    System.out.println("StringMotives: " + stringMotives);
    if (stringMotives != null) {
      if (stringMotives.equals(input.substring(input.length()-stringMotives.length()))) {  
        probCount = (count - 1);
      } else {  
        probCount = count;
      }
  ...
  void printProbabilities() { //prints conditional probabilities
  ...
  void calcSmoothing(double gval) { //smoothing algorithm
  ...
  String generate(String curr, ArrayList<Character> singleMotives, int lvalue) { //generate string in PST node class based on probability
    double rand = 0.0;
    double prob = 0;
    double nextProb = 0;
    boolean found = false;
