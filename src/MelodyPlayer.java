/* Stejara Dinulescu
 * Melody Player, taken from code by Dr. Courtney Brown
 * A class to take in a MIDI phrase played by the user from a MIDI synthesizer
 * Parses that MIDI message to then send to PST to train (see PSTNode + Tree class)
 */

import themidibus.*;
import processing.core.*;
import java.util.*;//   send the MIDI elsewhere to   play the notes
public class MelodyPlayer {
	...
	PApplet parent;//Parent - main class with processing functions
	...
	void setupMidi() {
		...
		outputMidiBus = new MidiBus(this, "AKM320", "IAC Bus 1");//or if   on   windows -  use a  windows virtual port
		//input, output
	}
	...
	void play() {// just do   nothing if   there is   no melody (pitches)
		if (!hasMelody)return;
		...
		if (play)last_time =  cur_time;//send a  note off to previous note --   TODO: control note-offs on staccato or legato, etc. values
		...
		if (note_index <  melody.size() && note_index > -1   && play) {
			...
			if   (hasRhythm)rhythm_multiplier = rhythm.get(note_index);
				note_index++;
			}
		}//reset note to 0
	...
	public void noteOn( int channel, int pitch, int velocity ) {
		System.out.println("Being called!: " + pitch);
		//convert pitch to a note character
		if (pitch == 60 || pitch == 72 || pitch == 84) {
			stringToTest = stringToTest + 'c';
		} else if (pitch == 62 || pitch == 74) {
			stringToTest = stringToTest + 'd';
		} else if (pitch == 64 || pitch == 76) {
			stringToTest = stringToTest + 'e';
		} else if (pitch == 65 || pitch == 77) {
			stringToTest = stringToTest + 'f';
		} else if (pitch == 55 || pitch == 67 || pitch == 79) {
			stringToTest = stringToTest + 'g';
		} else if (pitch == 57 || pitch == 69 || pitch == 81) {
			stringToTest = stringToTest + 'a';
		} else if (pitch == 59 || pitch == 71 || pitch == 83) {
			stringToTest = stringToTest + 'b';
		}
		
		//melody.add(pitch);
	}
	
	String getStringToTrain() { //returns string for PST
		return stringToTest;
	}
